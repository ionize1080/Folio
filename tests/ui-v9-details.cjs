...
(async()=>{
 ...
 await replace('Fixed frame overflow text. '.repeat(25));await stable();await page.waitForFunction(()=>!document.querySelector('#pe-warning-summary').hidden);
 assert(await page.locator('.page-edit-input').evaluate(el=>document.activeElement===el));assert(await page.locator('.page-edit-input').isEnabled());assert.equal((await draft()).model.allowOverflow,true);assert.equal(await page.locator('dialog[open]').count(),0);await page.screenshot({path:path.join(out,'v9-overflow-warning.png')});checks.push('fixed-frame overflow keeps an editable draft with a nonblocking status notification');
 ...
 await page.waitForFunction(()=>!document.querySelector('#pe-warning-summary').hidden&&/遮挡/.test(document.querySelector('#pe-warning-summary').textContent),null,{timeout:5000});
 assert.equal(await page.locator('.pe-notice').isHidden(),true);assert(await page.locator('.page-edit-input').isEnabled());await page.locator('#pe-warning-summary').click();assert.equal(await page.locator('.pe-notice').isHidden(),false);await page.locator('#pe-warning-summary').click();
