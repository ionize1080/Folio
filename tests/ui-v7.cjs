...
(async()=>{
 ...
 for(const filename of files){
  ...
  await page.evaluate(async({bytes,name})=>{await window.__qa.loadPDF(new Uint8Array(bytes),name);await window.__qa.surface.go(2);},{bytes,name:path.basename(filename)});
  await page.locator('[data-action="flow-edit"]').click();await page.waitForSelector('.page-edit-hit',{timeout:30000});
  assert.equal(await page.locator('#modal').evaluate(e=>e.open),false);checks.push(path.basename(filename)+': on-page mode, no dialog');
  ...
  await first.click();await page.waitForFunction(()=>!document.querySelector('.page-edit-ink').hidden,{timeout:30000});
  ...
  await page.waitForFunction(()=>/自动重排/.test(document.querySelector('#pe-status').textContent),{timeout:15000});
  await page.screenshot({path:path.join(out,'v7-'+(filename.includes('中国')?'monetary':filename.includes('绿色')?'green':'qdii')+'-editing.png')});
  await page.locator('#pe-done').click();await page.waitForFunction(()=>!window.__qa.S.flowEdit,{timeout:30000});
  const edited=await page.evaluate(async()=>Array.from(await window.__qa.S.pdf.getData()));fs.writeFileSync(path.join(out,'v7-'+(filename.includes('中国')?'monetary':filename.includes('绿色')?'green':'qdii')+'-edited.pdf'),Buffer.from(edited));
 ...
 assert.deepEqual(errors,[]);fs.writeFileSync(path.join(out,'v7-ui-report.json'),JSON.stringify({checks,errors},null,2));console.log(JSON.stringify({checks,errors}));
})().catch(async e=>{console.error(e);if(activePage){console.error('STATUS',await activePage.locator('#pe-status').textContent().catch(()=>''));console.error('ERRORS',errors);await activePage.screenshot({path:path.join(out,'v7-failure.png')});}process.exitCode=1;}).finally(async()=>{await browser?.close();server?.close();bridge.cancel();layout.close();});
