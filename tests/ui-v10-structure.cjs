...
(async()=>{
 ...
 await page.locator('.page-edit-hit').evaluateAll(es=>es.find(e=>e.getAttribute('aria-label').startsWith('Left column paragraph.')).click());await page.waitForFunction(()=>/自动重排完成/.test(document.querySelector('#pe-status').textContent));
 ...
 await page.waitForFunction(()=>/自动重排完成/.test(document.querySelector('#pe-status').textContent));await page.locator('#pe-more').click();await page.waitForSelector('#pe-chain-list li');assert.equal(await page.locator('#pe-chain-list li').count(),2);await page.screenshot({path:path.join(out,'v10-structure-chain.png')});await page.locator('#pe-close-properties').click();await page.locator('#pe-done').click();await page.waitForFunction(()=>!window.__qa.S.flowEdit);assert.equal(await page.evaluate(()=>window.__qa.S.nativeEdits.find(e=>e.model?.frames)?.model.frames.length),2);
