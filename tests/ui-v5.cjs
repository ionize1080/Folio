...
(async () => {
    ...
    if (req.method === "POST") {
      try {
        ...
        if (req.url === "/__job") result = await jobs.run(d);
          ...
          fs.writeFileSync(
            path.join(out, "v5-ui-saved.pdf"),
            Buffer.from(d.bytes),
          );
          result = { name: "v5-ui-saved.pdf", working: true };
        } else {
          if (d.ocrReference) {
  ...
  try {
    ...
    await page.locator("#tree").dispatchEvent("dragover", {
      dataTransfer: transfer,
      clientX: treeBox.x + 8,
      clientY: childBox.y + 10,
    });
    await page.waitForTimeout(100);
    await page.locator("#tree").dispatchEvent("drop", {
      dataTransfer: transfer,
      clientX: treeBox.x + 8,
      clientY: childBox.y + 10,
    });
    ...
    const expectedCurrentText = await page
      .locator("#ocr-correction")
      .inputValue();
    await page.evaluate(() => {
      const original = window.desktop.ocrJob;
      window.desktop.ocrJob = async (d) => {
        const r = await original(d);
        if (d.action === "page" && d.page === 3)
          await new Promise((resolve) => setTimeout(resolve, 350));
        return r;
      };
    });
    await page.locator("#ocr-review-page").fill("3");
    await page.locator("#ocr-review-page").press("Tab");
    ...
    await page.locator("#ocr-results").selectOption("0");
    assert.equal(
      await page.locator("#ocr-correction").inputValue(),
      expectedCurrentText,
    );
    check(
      "Delayed old-page reply cannot overwrite the current correction page",
    );
    await page.locator("#ocr-results").selectOption("0");
    await page.locator("#ocr-correction").fill("第二章 人工校对标题");
    // Deliberately omit explicit save: application/navigation must capture the draft.
    await page
      .locator(".native-properties")
      .evaluate((el) => (el.scrollTop = 0));
    await page.screenshot({ path: path.join(out, "v5-ocr.png") });
    check("Actual offline OCR with a pending correction draft");
    await page
      .locator("#ocr-correction")
      .fill("第二章 人工校对标题" + String.fromCodePoint(0x10ffff));
    await page.locator("#ocr-apply").click();
    await page.waitForSelector("#ocr-font-issues:not([hidden])");
    assert(await page.locator("#modal").evaluate((el) => el.open));
    assert(
      (await page.locator("#ocr-font-issues").textContent()).includes(
        "第 2 页",
      ),
    );
    check(
      "Missing glyph report stays visible in the OCR dialog before any application",
    );
    await page.locator("#ocr-correction").fill("第二章 人工校对标题");
    await page.locator("#ocr-apply").click();
    await page.waitForFunction(
    ...
    await page.waitForFunction(
      () =>
        document.querySelector("#doc-name").textContent === "v5-ui-saved.pdf" &&
        document.querySelector("#busy").hidden,
      null,
      { timeout: 180000 },
    );
    ...
    fs.writeFileSync(
      path.join(out, "v5-ui-report.json"),
    ...
    if (page)
      await page
        .screenshot({ path: path.join(out, "v5-ui-failure.png") })
        .catch(() => {});
    throw e;
  } finally {
    await jobs.stop();
