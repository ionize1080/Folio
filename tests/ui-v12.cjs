...
(async()=>{
 ...
 await page.evaluate(()=>{window.__cancelSave=false;const q=window.__qa;q.commit(q.S.nodes,{nativeEdits:[]});window.__historyCount=q.S.history.past.length;window.__nativeFailure=true;});
 await page.evaluate(async()=>{try{await window.__qa.actions.undo();}catch{};window.__nativeFailure=false;if(window.__qa.S.history.past.length!==window.__historyCount)throw Error('failed undo consumed history');});
 checks.push('Failed native undo preserves history and assets');
