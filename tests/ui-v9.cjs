...
(async()=>{
 ...
 await page.locator('.page-edit-hit[aria-label="Short title"]').click();await page.waitForFunction(()=>!document.querySelector('.page-edit-ink').hidden);
 ...
 await page.waitForFunction(()=>/自动重排完成/.test(document.querySelector('#pe-status').textContent),{timeout:20000});
 ...
 await page.locator('.page-edit-input').press('Control+z');await page.waitForFunction(()=>/自动重排完成/.test(document.querySelector('#pe-status').textContent));assert.equal((await page.locator('.page-edit-input').inputValue()).trim(),'Cell 1-1');
 await page.locator('.page-edit-input').press('Control+y');await page.waitForFunction(()=>/自动重排完成/.test(document.querySelector('#pe-status').textContent));checks.push('table growth preserves focus and undo/redo restores complete row geometry');
 await page.screenshot({path:path.join(out,'v9-table-growth.png')});
 await page.locator('.page-edit-input').press('Tab');await page.waitForFunction(()=>document.querySelector('.page-edit-input')?.value.trim()==='Cell 1-2'&&!document.querySelector('.page-edit-input').disabled&&/自动重排完成/.test(document.querySelector('#pe-status').textContent));
 await page.locator('.page-edit-input').evaluate(el=>{el.focus();el.select();document.execCommand('insertText',false,'Second cell changed');});await page.waitForFunction(()=>/自动重排完成/.test(document.querySelector('#pe-status').textContent));checks.push('Tab edits the next cell after row growth');
